...
def calculate_num_classes(data_dir):
    num_classes = 0
    for video_folder in os.listdir(data_dir):
        if video_folder.startswith("video_"):
            for object_folder in os.listdir(os.path.join(data_dir, video_folder)):
                if object_folder.startswith("object_"):
                    num_classes += 1
    return num_classes
...
def main(argv):
  ...
  if FLAGS.dataset=='CustomDataset':
      data_dir = FLAGS.data_dir  # 应该指向 `output` 文件夹

      # 遍历所有 `video_xxx/object_xxx` 文件夹，统计图像数量
      num_train_examples = 0
      num_eval_examples = 0
      for video_dir in glob.glob(os.path.join(data_dir, "video_*")):
          for object_dir in glob.glob(os.path.join(video_dir, "object_*")):
              images = glob.glob(os.path.join(object_dir, "*.jpg"))
              num_train_examples += len(images)  # 假设所有图片用于训练
      ...
      num_classes = calculate_num_classes(data_dir)
  ...
  if FLAGS.mode == 'eval':
    ...
    with strategy.scope():
      # Build input pipeline.
      if FLAGS.dataset=='CustomDataset':
        ds = data_lib.build_distributed_dataset(data_dir, FLAGS.train_batch_size, True, strategy, topology,num_classes)
      else:
